...
public class Interpreter {
	...
	private boolean outIsConsole = true;
	...
	public Interpreter(Bytecode bytecode, String[] programArgs) {
		...
		// nastav vystupni kontext dle parametru
		PrintStream out = System.out;

		// pokud je definovana systemova promenna outfile, zapisuju do souboru
		String outputTarget = System.getProperty("outfile");

		if (outputTarget != null) { // mam zadany vystupni soubor, pisu vystup
									// do nej
			try {
				outFile = new File(outputTarget);
	...
	private void processByte(byte b) throws InvalidInstructionException,
			...
			MethodNotFoundException, GlobalVariableTableDestroyException,
			ConstantRedefinitionException, ProgramExecutionException {
	...
	private void processProgramArgs(String[] args)
			throws ConstantRedefinitionException, VariableNotFoundException {
		if (args.length > 10) {
	...
	private void registerStaticMethods() {
		...
		try {
			methodTable.registerMethod(0, new PrintStaticMethod("print", ctx,
					bc, s, env));
			methodTable.registerMethod(1, new PrintlnStaticMethod("println",
					ctx, bc, s, env));
			methodTable.registerMethod(10, new ReadFileIntStaticMethod(
					"readfileint", bc, s, env));
			methodTable.registerMethod(11, new ReadFileIntArrStaticMethod(
					"readfileintarr", bc, s, env));
			methodTable.registerMethod(20, new WriteToFileStaticMethod(
					"writetofile", bc, s, env));
			methodTable.registerMethod(100, new LengthStaticMethod("length",
					bc, s, env));
	...
	/*
	 * pushsc [adresa] - vlozi na zasobnik Object string z tabulky konstant
	 */
	...
	/*
	 * call [adresa v method table] - volani staticke metody (metoda
	 * interpretru) na adrese v tabulce metod
	 */
	private void doCallInstruction() throws BytecodeOverflowException,
			MethodNotFoundException, EmptyStackPopException,
			StackOverflowException, ProgramExecutionException {
	...
	private void doConstdefInstruction() throws BytecodeOverflowException,
			ConstantRedefinitionException, VariableNotFoundException {
		...
		/*
		 * PUVODNI VERZE PRES POLE s definovanou delkou konstanty // definuj
		 * pole bytu, do ktereho se nactou byty konstanty byte[] arr = new
		 * byte[constSize];
		 * 
		 * for (int i = 0; i < arr.length; i++) { arr[i] = bc.nextByte(); }
		 * 
	...
	private void doStopInstruction() {
		...
		System.out.println("\nProgram ended with status 0 (OK) in " + secs
				+ " seconds.");
		if (!outIsConsole && outFile != null) {
			System.out.println("Output written to file " + outFile.getName());
		}
		if (Main.DEBUG) {
			s.printStack();
			env.printMemory();
