...
public class Environment implements IVariableTable {
	...
	/**
	 * Metoda je volana pri zanoreni do volane metody
	 * Vytvori novou tabulku promennych pro metodu a vlozi ji na vrchol zasobniku tabulek
	 */
	...
	/**
	 * Metoda je volana pri navratu z metody.
	 * Smaze nejvyssi tabulku promennych ze zasobniku.
	 * @throws GlobalVariableTableDestroyException 
	 */
	public void popLevel() throws GlobalVariableTableDestroyException {
		if(position == 0) throw new GlobalVariableTableDestroyException("Trying to remove global variables tables from environment");
	...
	public Object getValue(Integer address) throws VariableNotFoundException {
		...
		while(it.hasNext()) {
			...
			if(table.hasVariable(address)) {
				Util.debugMsg("  Variable on adress "+address + " found in level " + level);
				return table.getValue(address);
			}
			level--;
		}
		throw new VariableNotFoundException("   Variable with address " + address + " not found in any variable table");
	}
	...
	public boolean hasVariable(Integer address) {
		...
		while(it.hasNext()) {
			table = it.next();
			if(table.hasVariable(address)) return true;
		}
	...
	/**
	 * Vraci nula (GLOBAL_DEPTH), pokud jsem na globalni urovni (globalni tabulka promennych)
	 * S kazdou volanou metodou se zvysi o 1
	 * @return
	 */
	public int depth() {
